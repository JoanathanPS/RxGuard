...
Phase 1 adds auth (register/login → JWT, RBAC), CORS for the dev frontend, and
a startup hook that guarantees this service's Postgres schema exists (the
Alembic migration is authoritative; the hook only helps host-local dev).
"""
...
from user_app.api.router import api_router
...
app = FastAPI(
    title="RxGuard User Service",
    description="Registration, login, JWT issuance, and role management.",
    version="0.1.0",
    lifespan=lifespan,
)
